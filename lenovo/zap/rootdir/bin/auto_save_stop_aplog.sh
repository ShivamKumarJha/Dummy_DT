...
# This shell will record it history to file /data/local/newlog/aplog/auto_save_stop_aplog.history.
...
if [ -n "$(getprop persist.sys.firstbootfinish)" ]; then
  if [ $(getprop persist.sys.firstbootfinish) = 1 ]; then
    echo "persist.sys.firstbootfinish is 1, exit." >> $LOGFILE
    return
  fi
else
  echo "persist.sys.firstbootfinish is not set, set to default value 1" >> $LOGFILE
  setprop persist.sys.firstbootfinish 1
fi
