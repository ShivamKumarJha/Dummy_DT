...
function find_laser_offset()
{
    for file in `ls $1`;
    do
        if [ -f "$1/$file/offset" ]; then
            laser_offset_path="$1/$file/offset"
            break;
        else
            echo "$1/$file has no file offset"
        fi
    done
}
find_laser_offset "/sys/devices/virtual/input"
echo "$laser_offset_path"
