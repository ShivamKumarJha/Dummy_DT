...
verify_oem()
{
    ...
    if [ "x$product" != "x$oem_product" ]; then
        do_hab_error "OEM_PRODUCT_CHECK"
    fi

    if [ "x$cid" != "x$oem_cid" ]; then
        do_hab_error "OEM_CID_CHECK"
    fi

    if [ "x$csv" != "x$oem_csv" ]; then
...
verify_vendor()
{
    ...
    value_from_key "HAB_PRODUCT" "$vendor_file"
    vendor_product=$value
    value_from_key "HAB_CID" "$vendor_file"
    vendor_cid=$value
    value_from_key "HAB_SECURITY_VERSION" "$vendor_file"
    vendor_csv=$value
    ...
    if [ "x$product" != "x$vendor_product" ]; then
        do_hab_error "VENDOR_PRODUCT_CHECK"
    fi

    if [ "x$cid" != "x$vendor_cid" ]; then
        do_hab_error "VENDOR_CID_CHECK"
    fi

    if [ "x$csv" != "x$vendor_csv" ]; then
