...
case "$fqcaudio" in
    "11")   # mic 1 to (digital) headset recv loopback test start
		...
		/vendor/bin/tinyhostless -D 0 -P 39 -C 5 -p 1024 -n 2 -c 1 -r 48000 -T 16 > /dev/null 2>&1 &
		;;
    ...
    "41")   # (digital) headset mic to headset recv loopback test start
		...
		/vendor/bin/tinyhostless -D 0 -P 39 -C 39 -p 1024 -n 2 -c 1 -r 48000 -T 16 > /dev/null 2>&1 &
		;;
    ...
    "51")   # mic 1 to (analog) headset recv loopback test start
		/vendor/bin/tinymix2 'Audio_MicToHeadset_ON' > /dev/null 2>&1
		;;
    "50")   # mic 1 to (analog) headset recv loopback test end		
		/vendor/bin/tinymix2 'Audio_MicToHeadset_OFF' > /dev/null 2>&1
		;;
    "61")   # mic 2 to (analog) headset recv loopback test start
		/vendor/bin/tinymix2 'Audio_Mic2ToHeadset_ON' > /dev/null 2>&1
		;;
    "60")   # mic 2 to (analog) headset recv loopback test end
		/vendor/bin/tinymix2 'Audio_Mic2ToHeadset_OFF' > /dev/null 2>&1
		;;
    ...
    "70")   # mic 3 to (analog) headset recv loopback test end
		...
		/system/bin/tinymix 'RX INT1 DEM MUX' 'NORMAL_DSM_OUT' > /dev/null 2>&1
		/system/bin/tinymix 'RX INT2 DEM MUX' 'NORMAL_DSM_OUT' > /dev/null 2>&1
		;;
    "81")   # (analog) headset mic to headset recv loopback test start
		/vendor/bin/tinymix2 'Audio_HeadsetMicToHeadset_ON' > /dev/null 2>&1
		;;
    "80")   # (analog) headset mic to headset recv loopback test end
		/vendor/bin/tinymix2 'Audio_HeadsetMicToHeadset_OFF' > /dev/null 2>&1
		;;
    *)
		;; #Do nothing
esac
